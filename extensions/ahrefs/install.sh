#!/usr/bin/env bash
# Codex SEO — Ahrefs extension installer.
#
# Wires the official @ahrefs/mcp server into ~/.codex/settings.json and
# copies the seo-ahrefs mirror skill into ~/.codex/skills/.
#
...
main() {
    ...
    echo "════════════════════════════════════════"
    echo "║   Codex SEO — Ahrefs extension      ║"
    echo "════════════════════════════════════════"
    ...
    if [ ! -d "${SKILL_DIR}/seo" ]; then
        echo "✗ codex-seo base plugin not installed."
    ...
    # Merge MCP config into ~/.codex/settings.json atomically.
...
PY
    ...
    echo
    echo "Done. Open a new Codex session and run:"
