...
server into your Codex session so the `seo-ahrefs` skill can call
live Ahrefs data.
...
4. Copies `skills/seo-ahrefs/SKILL.md` into `~/.codex/skills/seo-ahrefs/`.
5. Atomically writes `mcpServers.ahrefs` into `~/.codex/settings.json`
...
`scripts/dataforseo_costs.py` cost tracker shipped with codex-seo
