...
Before gathering fresh context, check `.seo-cache/` for reusable site and page data. Use `.seo-cache/site-meta.json`, `.seo-cache/audit-scores.json`, and `.seo-cache/pages/{url-slug}/page-analysis.json` when present and valid. If the user says "refresh" or "re-run", ignore cache and gather fresh evidence.
